...
public class DorisTypeConverter extends JdbcTypeConverter {
    ...
    static final String VARCHAR_REGEX = "^varchar\\((\\d+)\\)$";

    @Override
    public Type toGravitino(JdbcTypeBean typeBean) {
        Pattern pattern = Pattern.compile(VARCHAR_REGEX);
        Matcher matcher = pattern.matcher(typeBean.getTypeName().toLowerCase());
        if (matcher.matches()) {
            String length = matcher.group(1);  // group(1) 是括号中的第一个捕获组，即长度
            return Types.VarCharType.of(Integer.parseInt(length));
        }
        switch (typeBean.getTypeName().toLowerCase()) {
            ...
            case DECIMAL:
                return Types.DecimalType.of(Integer.parseInt(typeBean.getColumnSize()), Integer.parseInt(typeBean.getScale()));
            case DATE:
